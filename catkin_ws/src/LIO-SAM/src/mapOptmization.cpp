...
float adaptiveResidualK(float dis_point)
{
    float k = 0.9f;

    if (dis_point < 30.0f) {
        float ratio_dis = std::round((30.0f - dis_point) / 10.0f);
        k = (8.0f + 2.0f * ratio_dis) / (9.0f + 2.0f * ratio_dis);
    } else if (dis_point > 60.0f) {
        float ratio_dis = std::ceil((dis_point - 60.0f) / 20.0f);
        if (ratio_dis > 3.0f)
            ratio_dis = 3.0f;
        k = (8.0f - ratio_dis) / (9.0f - ratio_dis);
    }
        ...
        for (int i = 0; i < laserCloudSurfLastDSNum; i++)
        {
            ...
            if (pointSearchSqDis[4] < 1.0) {
                ...
                if (planeValid) {
                    ...
                    float dis_point = sqrt(pointOri.x * pointOri.x
                                      + pointOri.y * pointOri.y
                                      + pointOri.z * pointOri.z);

                    float k = adaptiveResidualK(dis_point);

                    float s = 1 - k * fabs(pd2) / sqrt(sqrt(pointSel.x * pointSel.x
